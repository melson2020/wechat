...
Page({
   ...
   checkLoginStatus: function () {
      let that = this;
      let res=app.globalData.userInfo&&wx.getStorageSync('token')
      that.setData({
         'hasLogin': res
      })
      return res
   ...
   checkBoxClick: function (option) {
      ...
      that.setData({
         [temp]: !that.data.shoppingCartList[index].checked
      })
      that.setCheckAll()
      that.SumCheckedTotalPrice()
   },
   ...
   setCheckAll: function () {
      ...
      let ischeckedAll = that.data.shoppingCartList.filter((item) => {
   ...
   slideButtonTap: function (option) {
      ...
      var that = this
      var token = wx.getStorageSync('token')
      http.postRequest('/shoppingCart/deleteShoppingCart', {
         'token': token,
         data: {
            id: option.currentTarget.id
         }
      }).then((response) => {
         if (response.data.resultStatus === 1) {
            var index = option.currentTarget.dataset.index
